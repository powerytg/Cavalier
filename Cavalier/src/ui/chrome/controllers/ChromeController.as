	...
	public class ChromeController extends EventDispatcher
	{
		...
		public function initialize():void{
			...
			chrome.addEventListener(ChromeEvent.CHANGE_BACKGROUND, function(evt:ChromeEvent):void{
				evt.stopPropagation();
				if(!chrome.background.visible)
					chrome.background.visible = true;
				
				chrome.background.source = evt.backgroundFace;
			});
			
			// Wallpaper events
			chrome.addEventListener(ChromeEvent.RESET_BACKGROUND, function(evt:ChromeEvent):void{
				evt.stopPropagation();
				
				if(!chrome.background.visible)
					chrome.background.visible = true;
				
				chrome.background.source = chrome.background.defaultBackground;
			});
			
			chrome.addEventListener(ChromeEvent.HIDE_BACKGROUND, function(evt:ChromeEvent):void{
				evt.stopPropagation();
				chrome.background.visible = false;
			});
			
			chrome.addEventListener(ChromeEvent.SHOW_BACKGROUND, function(evt:ChromeEvent):void{
				trace('show');
				evt.stopPropagation();
				chrome.background.visible = true;
			});
